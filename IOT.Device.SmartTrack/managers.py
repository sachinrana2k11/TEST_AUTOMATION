...
class cloudmanager(object):
    ...
    def __init__(self,log):
        ...
        #self.block_blob_service.create_container('mycontainer')
    ...
    def update_synced(self,msg_id):
     try:   
        if (int(ConfigHelper.debugmode) == 1):
            ...
            query = devicedata.delete().where((devicedata.id == msg_id))
            query.execute()
    ...
    def update_file_synced(self,filename):
          filename_new = ConfigHelper.cameraconfig.image_file_path + "sync_" + filename
        ...
        try:
            ...
            while self.isrunning:
                  ...
                  for data in devicedata().select().order_by(devicedata.datestamp.desc() and devicedata.timestamp.asc()).where(devicedata.synced == 0).limit(int(ConfigHelper.datasync.batch_size)):
                        ...
                        message_to_send= json.dumps(model_to_dict(data))
                        #message_to_send = IoTHubMessage(json.dumps(model_to_dict(data)))
                        #self.logger.info("object feed up with jason object..")
                        #message_to_send.message_id = data.id
                        #message_to_send.correlation_id = data.id'''
                        msg_id = data.id
                        ...
                        try:
                          self.sbs.send_event('test-dev',message_to_send)#sending to event hub 
                          self.update_synced(msg_id)#updating sync
                        except:
                          e = sys.exc_info()[0]
                          self.events.cloud_exception(e)
                          self.logger.error("Exception in send_data_to_eventhub:"+str(e))
                          continue
                        #cloudmanager.client.send_event_async(message, self.send_confirmation_callback, 0) #iot hub sending 
                  ...
                  if(ConfigHelper.cameraconfig.feed_enabled):
                          ...
                          for file in list(glob.glob(directory)):
                            ...
                            self.block_blob_service.create_blob_from_path('mycontainer/testdevice',filename,file,content_settings=ContentSettings(content_type='image/jpeg'))
                            ...
                            self.update_file_synced(filename)
                            break
